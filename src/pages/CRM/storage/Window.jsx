...
class Window {
  ...
  editLoading = (name, value) => {
    if (name === 'table') { return this.loadingTable = value }
    if (name === 'form') { return this.loadingForm = value }
  };
  ...
  updateWindowForm = (value = null, id = null) => {
    if (value === null) { return this.windowForm = !this.windowForm };
  ...
  updateWindowDelete = (id = null) => {
    id === null ? (this.clientId = null, this.windowDelete = !this.windowDelete)
      : (this.clientId = id, this.windowDelete = !this.windowDelete)
  };

  updateContactsClient = (value) => { this.contactsClient.push(value) };
  ...
  checkDataClient = (name, value) => {
    switch (name) {
      ...
      case 'LastName':
        ...
        } else { return this.client.lastName = '' };
    }
  };
  ...
  checkData = (element = null) => {
    if (element === 'Surname') {
      this.client.surname !== '' &&
        (this.listError = this.listError.filter(el => el !== 'Surname'));
    }
    if (element === 'Name') {
      this.client.name !== '' &&
        (this.listError = this.listError.filter(el => el !== 'Name'));
    }
    if (element === 'LastName') {
      this.client.lastName === '' &&
        (this.listError = this.listError.filter(el => el !== 'LastName'));
    }
    ...
    if (element === null) {
      this.client.surname === '' && !this.listError.includes('Surname') &&
        this.listError.push('Surname');

      this.client.name === '' && !this.listError.includes('Name')
        && this.listError.push('Name');

      this.client.lastName === false && !this.listError.includes('LastName')
        && this.listError.push('LastName');

      this.contactsClient.map((el) => {
        (el.contact.value === '' || el.contact.value === false) && !this.listError.includes(el.id)
  ...
  getErrors = (value, i) => {
    switch (value) {
      case 'Surname':
        return <span key={i} className="crm-bug">Введи корректную Фамилию</span>;

      case 'Name':
        return <span key={i} className="crm-bug">Введи корректное Имя</span>;

      case 'LastName':
        return <span key={i} className="crm-bug">Введи корректное Отчество</span>;

      case 'Телефон':
        return <span key={i} className="crm-bug">Введи корректный номер Телефона</span>;

      case 'Email':
        return <span key={i} className="crm-bug">Введи корректный Email</span>;

      case 'Facebook':
        return <span key={i} className="crm-bug">Введи корректное id Facebook</span>;

      case 'Vk':
        return <span key={i} className="crm-bug">Введи корректное id Vk</span>;

      case 'Другое':
        return <span key={i} className="crm-bug">Введи корректные данные</span>;
    }
  };
  ...
  updateData = (value) => {
    ...
    if (value === 'create') { return Storage.createClient(this.client) };
    if (value === 'edit') { return Storage.editClient(this.client) };
  };
}
